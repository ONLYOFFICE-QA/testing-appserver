...
shared_examples_for 'documents_img_media_archives_filter' do |folder, files_array, folder_name|
  it "Images filter works in #{folder}" do
    ...
    images = TestingAppServer::HelperFiles.files_by_extension(files_array, :jpg)
  ...
  it "Media filter works in #{folder}" do
    ...
    audios = TestingAppServer::HelperFiles.files_by_extension(files_array, :mp3)
  ...
  it "Archives filter works in #{folder}" do
    ...
    archives = TestingAppServer::HelperFiles.files_by_extension(files_array, :zip)
