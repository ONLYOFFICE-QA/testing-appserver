...
shared_examples_for 'documents_file_menu' do |product, folder, api_admin, new_document, version_document, move_document,
  copy_document, create_copy_document, rename_document, delete_document, folder_name|
  describe 'File menu' do
    it "[#{product}][#{folder}][File Menu] `Edit` file opens correct file" do
    ...
    it "[#{product}][#{folder}][File Menu] `Share external link` works" do
    ...
    describe 'Version' do
      it "[#{product}][#{folder}][File Menu] Finalize version appears lock" do
        documents_page.file_settings(new_document, :finalize_version)
        expect(documents_page.file_version(new_document)).to eq(2)
      end

      it "[#{product}][#{folder}][File Menu] New version appears in version history" do
      ...
      it "[#{product}][#{folder}][File Menu] Download first version" do
        documents_page.file_settings(version_document, :version_history)
    ...
    it "[#{product}][#{folder}][File Menu] Download button works" do
    ...
    describe 'Move or copy' do
      it "[#{product}][#{folder}][File Menu] Move to button works" do
      ...
      it "[#{product}][#{folder}][File Menu] Copy to button work" do
      ...
      it "[#{product}][#{folder}][File Menu] Create a copy button work" do
    ...
    it "[#{product}][#{folder}][File Menu] Rename button works" do
    ...
    it "[#{product}][#{folder}][File Menu] Delete button works" do
