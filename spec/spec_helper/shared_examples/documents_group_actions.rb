...
shared_examples_for 'documents_group_actions' do |document_name, move_document, delete_document, folder_name|
  ...
  it '[My Documents] `Download as` group button works' do
    pending('https://bugzilla.onlyoffice.com/show_bug.cgi?id=56394') if @test.product == :appserver
    documents_page.check_file_checkbox(document_name)
  ...
  it '[My Documents] `Copy` to folder group button works' do
    documents_page.check_file_checkbox(document_name)
